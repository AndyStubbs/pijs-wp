...
window.piExtra.util = ( function () {
	...
	function convertToColor( color ) {
		...
		if( pi.util.isArray( color ) && color.length > 2 ) {
...
pi._.addCommand( "onkey", onkey, false, false,
	[ "key", "mode", "fn", "once" ]
);
function onkey( args ) {
	...
	// Validate fn
	if( ! pi.util.isFunction( fn ) ) {
		m_piData.log( "onkey: fn is not a valid function." );
		return;
	}
...
function offkey( args ) {
	...
	// Validate fn
	if( ! pi.util.isFunction( fn ) ) {
		m_piData.log( "offkey: fn is not a valid function." );
		return;
	}
...
pi._.addCommand( "setActionKey", setActionKey, false, false,
	[ "key", "isEnabled" ]
);
pi._.addSetting( "actionKey", setActionKey, false, [ "key", "isEnabled" ] );
function setActionKey( args ) {
	...
	if( ! ( typeof key === "string" || pi.util.isInteger( key ) ) ) {
		m_piData.log(
...
function showPrompt( screenData, hideCursor ) {
	...
	if( m_inputs.length > 0 && m_inputIndex < m_inputs.length ) {
		...
		pos = m_piData.commands.getPos( input.screenData );
		if(pos.row >= m_piData.commands.getRows( input.screenData ) ) {			
			m_piData.commands.print( input.screenData, [ "", false ] );	
			m_piData.commands.setPos( input.screenData, [ pos.col, pos.row - 1] );
			pos = m_piData.commands.getPos( input.screenData );
		}
		
		// Get the background pixels
		posPx = m_piData.commands.getPosPx( input.screenData );
		...
		if( ! m_promptBackground ) {
			m_promptBackground = m_piData.commands.get( input.screenData,
			...
			m_piData.commands.put( input.screenData,
		...
		// Print the prompt		
		m_piData.commands.print( input.screenData, [ msg, true ] );		
		m_piData.commands.setPos( input.screenData, [ pos.col, pos.row ] );
		m_piData.commands.render( input.screenData );
...
function showOnscreenKeyboard( screenData, input ) {
	...
	pos = m_piData.commands.getPos( screenData );
	m_piData.commands.setPos( screenData, [ 0, pos.row + 1 ] );
	...
	if( m_keyboard.background ) {
		m_piData.commands.put(
			screenData, [
				m_keyboard.background, x, y, true
			]
		);
	} else {
		m_keyboard.background = m_piData.commands.get(
			screenData, [ x, y, x + width, y + height ]
		);
	}

	// Print the keyboard
	hitBoxes = m_piData.commands.printTable(
		screenData, [
			keys,
			m_keyboard.format
		]
	);
	m_piData.commands.setPos( screenData, [ pos.col, pos.row ] );

	if( ! m_keyboard.eventsLoaded ) {

		if( m_piData.isTouchScreen ) {
			input.keyboardDetected = false;
		}

		// Add OnPress Events
		for( i = 0; i < hitBoxes.length; i++ ) {
			m_piData.commands.onpress( screenData, [
				"down",
...
function clearOnscreenKeyboardEvents( screenData ) {
	...
	if( m_keyboard.eventsLoaded ) {
		...
		// Add OnPress Events
		for( i = 0; i < hitBoxes.length; i++ ) {
			m_piData.commands.offpress( screenData, [
				"down",
				onscreenKeyboardOnPress,
				false,
				hitBoxes[ i ].pixels
...
function onscreenKeyboardOnPress( data, keyData ) {
	...
	if( key === "Enter" ) {
		...
		m_piData.commands.setColor( keyData.screenData, [ 15 ] );
		m_piData.commands.rect( keyData.screenData, [
			keyData.pixels.x, keyData.pixels.y,
			keyData.pixels.width + 1, keyData.pixels.height + 1
		] );
		m_piData.commands.setColor( keyData.screenData, [ 7 ] );
	}
}
...
function input( screenData, args ) {
	...
	if( typeof prompt !== "string" ) {
		m_piData.log( "input: prompt must be a string." );
		return;
	}

	// Validate callback
	if( callback != null && ! pi.util.isFunction( callback ) ) {
		m_piData.log( "input: callback must be a function." );
		return;
	}
	...
	if( typeof onscreenKeyboard !== "string" ) {
		m_piData.log( "input: onscreenKeyboard must be a string." );
		return;
	}
...
function collectInput( event ) {
	...
	if( event.keyCode === 13 ) {
		...
		m_piData.commands.print( input.screenData, [ "" ] );
		triggerReady( input );
		m_inputIndex += 1;
...
pi._.addCommand( "setInputCursor", setInputCursor, false, true,
	[ "cursor" ]
);
...
function setInputCursor( screenData, args ) {
	...
	if( typeof cursor !== "string" ) {
		m_piData.log( "setInputCursor: cursor must be a string or integer." );
		return;
	}
...
function ongamepad( args ) {
	...
	m_piData.commands.onevent( mode, fn, once, false, m_Modes, "ongamepad",
...
function offgamepad( args ) {
	...
	m_piData.commands.offevent( mode, fn, m_Modes, "offgamepad", m_events,
		extraData.extraId );
}
...
function getExtraData( name, items, gamepadIndex, mode ) {
	...
	if( isNaN( gamepadIndex ) || gamepadIndex < 0 ) {
		m_piData.log( name + ": gamepadIndex is not a valid index number." );
		return;
	}
	...
	if( mode === "connect" || mode === "disconnect" ) {
		...
		if( ! pi.util.isInteger( items ) || items < 0 ) {
			m_piData.log( name + ": items is not a valid axis index." );
			return;
		}
		items = [ items ];
	} else {
		...
		if( ! pi.util.isArray( items ) ) {
			m_piData.log( name + ": items is not a valid array." );
			return;
		}
...
function triggerAxes( gamepadIndex, axis, eventName ) {
	...
	if( axes.length > axis ) {
		if(
			...
			m_piData.commands.triggerEventListeners(
				eventName, axes[ axis ],
				m_events
			);
		}
...
// Point command
pi._.addCommand( "point", point, false, true, [ "x", "y" ] );
...
function pxBezier( screenData, args ) {
	...
	if( isNaN( xStart ) || isNaN( yStart ) ||
		...
		m_piData.log( "bezier: Argument's xStart, yStart, x1, y1, x2, y2, xEnd, and yEnd" +
			" must be numbers." );
		return;
	}
...
( function () {
	...
	function printTable( screenData, args ) {
		...
		isCentered = !!( args[ 3 ] );
		if( ! pi.util.isArray( items ) ) {
			m_piData.log( "printTable: items must be an array" );
			return;
		}
		...
		if( isFormatted ) {
			...
			width = m_piData.commands.getCols( screenData );
	...
	function buildStandardTable( screenData, items, width, borders ) {
		...
		boxes = [];
		font = screenData.printCursor.font;
		startPos = m_piData.commands.getPos( screenData );
		cellHeight = 2;
		...
		m_piData.commands.print( screenData, [ msg ] );
	...
	function buildFormatedTable(
		...
		if( isCentered ) {
			pos.col = Math.floor( ( m_piData.commands.getCols( screenData ) -
				tableFormat[ 0 ].length ) / 2
			);
		}

		// Create the padding
		padding = pi.util.pad( "", pos.col, " " );
		m_piData.commands.setPos( screenData, [ 0, pos.row ] );
		...
		posAfter = m_piData.commands.getPos( screenData );
		...
		m_piData.commands.setPos( screenData,
	...
	function printItem( screenData, box, msg ) {
		...
		if( isVertical ) {
			...
			for( row = startRow; row <= height + startRow; row += 1 ) {
				m_piData.commands.setPos( screenData, [ col, row ] );
				m_piData.commands.print( screenData,
			...
			m_piData.commands.setPos( screenData, [ col, row ] );
			m_piData.commands.print( screenData, [ msg, true ] );
		}
		m_piData.commands.setPos( screenData, [ pos.col, pos.row ] );
	}
...
( function () {
	...
	function play( args ) {
		...
		} else if( pi.util.isInteger( playString ) ) {
			trackId = playString;
		...
		if( m_tracks[ trackId ] ) {
			...
			m_piData.log( "Playstring needs to be a string or a integer" );
		}
	...
	function stopPlay( args ) {
		...
		if( trackId == null ) {
			for( i = 0; i < m_tracks.length; i++ ) {
				for( j = 0; j < m_tracks[ i ].sounds.length; j++ ) {
					m_piData.commands.stopSound( [ m_tracks[ i ].sounds[ j ] ] );
				}
			}
			m_tracks = [];
		...
		// Validate soundId
		if( ! m_tracks[ trackId ] ) {
			m_piData.log( "stopPlay: track ID " + trackId + " not found." );
			return;
		}
		...
		for( i = 0; i < trackIds.length; i++ ) {
			for( j = 0; j < m_tracks[ trackIds[ i ] ].sounds.length; j++ ) {
				m_piData.commands.stopSound( [ m_tracks[ trackIds[ i ] ].sounds[ j ] ] );
			}
...
pi._.data.commands.setDefaultColor( [ 7 ] );
